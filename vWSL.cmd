...
ECHO:& ECHO [%TIME:~0,8%] Installing base (~1m00s)
...
%GO% "apt-get update ; DEBIAN_FRONTEND=noninteractive apt-get -y dist-upgrade --allow-remove-essential ; apt-get -y install git"
...
%GO% "apt-get -y purge mesa-vulkan-drivers *gnustep* libgc1c2 libobjc4 powermgmt-base unar ; apt-get -y clean" > ".\logs\%TIME:~0,2%%TIME:~3,2%%TIME:~6,2% Final clean-up.log"

SET /A SESMAN = %RDPPRT% - 50
%GO% "which schtasks.exe" > "%TEMP%\SCHT.tmp" & set /p SCHT=<"%TEMP%\SCHT.tmp"
%GO% "sed -i 's#SCHT#%SCHT%#g' /tmp/vWSL/dist/usr/local/bin/reboot ; sed -i 's#DISTRO#%DISTRO%#g' /tmp/vWSL/dist/usr/local/bin/reboot"
